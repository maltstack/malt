...
impl RegistryClient {
    pub fn new() -> Result<Self, RegistryError> {
        Ok(Self {
            client: Client::builder().user_agent("malt-image/0.1").build()?,
        })
    }
    ...
    pub fn fetch_manifest_descriptor(
        &self,
        ...
        if bytes.len() as u64 != descriptor.size {
            return Err(RegistryError::ContentLength {
    ...
    pub fn download_blob(
        ...
        let mut verifier = TeeReader {
            source: &mut response,
            target: output,
        };
        verify_reader(&mut verifier, &descriptor.digest, descriptor.size)
    ...
    fn get_with_bearer(&self, url: &str, accept: Option<&str>) -> Result<Response, RegistryError> {
        ...
        if initial.status() != StatusCode::UNAUTHORIZED {
            return Err(RegistryError::Status(initial.status()));
        }
        let header = initial
            .headers()
            .get(WWW_AUTHENTICATE)
            .and_then(|value| value.to_str().ok())
            .ok_or(RegistryError::UnsupportedChallenge)?;
        ...
        if response.status().is_success() {
            ...
            Err(RegistryError::Status(response.status()))
        }
    }
...
fn bearer_token_url(header: &str) -> Result<String, RegistryError> {
    ...
    if !realm.starts_with("https://") {
        return Err(RegistryError::MalformedChallenge);
    }
