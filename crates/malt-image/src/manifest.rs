...
pub fn select_windows_amd64(
    index_json: &[u8],
    required_os_version: Option<&str>,
) -> Result<Descriptor, ManifestError> {
    let index: ManifestIndex = serde_json::from_slice(index_json)?;
    if index.schema_version != 2 {
        return Err(ManifestError::SchemaVersion(index.schema_version));
    }
    ...
    let candidates = index
        ...
        .filter(|descriptor| {
            descriptor
                .platform
                .as_ref()
                .is_some_and(Platform::is_windows_amd64)
                && required_os_version
                    .map(|version| {
                        descriptor
                            .platform
...
mod tests {
    ...
    fn select_rejects_linux_only_index() {
        let json = format!(
            r#"{{"schemaVersion":2,"mediaType":"{OCI_IMAGE_INDEX}","manifests":[{{"mediaType":"{OCI_IMAGE_MANIFEST}","digest":"{DIGEST}","size":3,"platform":{{"os":"linux","architecture":"amd64"}}}}]}}"#
        );
        assert!(matches!(
            select_windows_amd64(json.as_bytes(), None),
            Err(ManifestError::NoWindowsAmd64)
        ));
    }
    ...
    fn select_rejects_wrong_windows_architecture_and_malformed_json() {
        let json = format!(
            r#"{{"schemaVersion":2,"manifests":[{{"mediaType":"{OCI_IMAGE_MANIFEST}","digest":"{DIGEST}","size":3,"platform":{{"os":"windows","architecture":"arm64"}}}}]}}"#
        );
        assert!(matches!(
            select_windows_amd64(json.as_bytes(), None),
            Err(ManifestError::NoWindowsAmd64)
        ...
        assert!(matches!(
            select_windows_amd64(b"{not-json", None),
    ...
    fn select_rejects_ambiguous_windows_variants_without_policy() {
        let json = format!(
            r#"{{"schemaVersion":2,"manifests":[{{"mediaType":"{OCI_IMAGE_MANIFEST}","digest":"{DIGEST}","size":3,"platform":{{"os":"windows","architecture":"amd64","os.version":"20348"}}}},{{"mediaType":"{OCI_IMAGE_MANIFEST}","digest":"{DIGEST}","size":3,"platform":{{"os":"windows","architecture":"amd64","os.version":"26100"}}}}]}}"#
        );
        assert!(matches!(
            select_windows_amd64(json.as_bytes(), None),
            Err(ManifestError::AmbiguousWindowsAmd64)
        ));
        assert_eq!(
            select_windows_amd64(json.as_bytes(), Some("26100"))
                .expect("select")
                .platform
                .expect("platform")
                .os_version
