...
impl FromStr for Digest {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        let hexadecimal = value.strip_prefix("sha256:").ok_or(DigestError::UnsupportedAlgorithm)?;
...
mod tests {
    ...
    fn canonical_digest_round_trips() {
        ...
        assert_eq!(digest.to_string(), "sha256:ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad");
    }

    #[test]
    fn verification_checks_digest_and_size() {
        let digest: Digest = "sha256:ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad".parse().expect("digest");
        assert_eq!(verify_reader(&mut Cursor::new(b"abc"), &digest, 3).expect("verify"), 3);
        assert!(matches!(verify_reader(&mut Cursor::new(b"abc"), &digest, 2), Err(DigestError::SizeMismatch { .. })));
    }
}
