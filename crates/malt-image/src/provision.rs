...
use crate::{parse_image_manifest, select_windows_amd64, Descriptor, Digest, ImageManifest, ImageRecord, ImageReference, ImageStore, ManifestError, Platform, RegistryClient, RegistryError, StoreError};
...
pub fn acquire_public_windows_image(store: &ImageStore, reference_text: &str) -> Result<ImageRecord, ProvisionError> {
    ...
    let (manifest_digest, manifest, platform) = match parse_image_manifest(&initial) {
        Ok(manifest) => {
            let (digest, _) = Digest::from_reader(&mut &initial[..]).map_err(|error| ProvisionError::Registry(RegistryError::Verify(error)))?;
            let platform = fetch_platform(&client, &reference, &manifest.config)?;
            (digest, manifest, platform)
        }
        Err(ManifestError::MissingConfig | ManifestError::MissingLayers | ManifestError::MediaType(_) | ManifestError::SchemaVersion(_)) => {
            ...
            let platform = selected.platform.ok_or(ProvisionError::UnsupportedPlatform)?;
            (selected.digest, manifest, platform)
        }
        Err(error) => return Err(ProvisionError::Manifest(error)),
    };
    if !platform.is_windows_amd64() { return Err(ProvisionError::UnsupportedPlatform); }
    let staging = store.staging_path(&format!("acquire-{}", manifest_digest.to_string().trim_start_matches("sha256:")))?;
    ...
    let record = ImageRecord { manifest_digest, source_reference: reference.to_string(), platform, manifest, prepared: false };
...
fn fetch_platform(client: &RegistryClient, reference: &ImageReference, descriptor: &Descriptor) -> Result<Platform, ProvisionError> {
    let mut bytes = Vec::new();
    client.download_blob(reference, descriptor, &mut bytes)?;
    let config: ImageConfig = serde_json::from_slice(&bytes)?;
    Ok(Platform { os: config.os, architecture: config.architecture, os_version: config.os_version })
}

fn acquire_blobs(client: &RegistryClient, reference: &ImageReference, store: &ImageStore, staging: &std::path::Path, manifest: &ImageManifest) -> Result<(), ProvisionError> {
