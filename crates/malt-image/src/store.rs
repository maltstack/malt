...
impl ImageStore {
    ...
    pub fn record_path(&self, digest: &Digest) -> PathBuf {
        self.root.join("records").join(format!(
            "{}.json",
    ...
    pub fn publish_blob(&self, digest: &Digest, staged: &Path) -> Result<(), StoreError> {
        ...
        let parent = destination
            .parent()
            .ok_or_else(|| StoreError::Io(std::io::Error::other("MALT blob path has no parent")))?;
    ...
    pub fn publish_record(&self, record: &ImageRecord) -> Result<(), StoreError> {
        ...
        if destination.exists() {
            return Err(StoreError::Exists(record.manifest_digest.clone()));
        }
    ...
    pub fn replace_record(&self, record: &ImageRecord) -> Result<(), StoreError> {
        let destination = self.record_path(&record.manifest_digest);
        if !destination.exists() {
            return Err(StoreError::Missing(record.manifest_digest.clone()));
        }
        let temporary = destination.with_extension("json.tmp");
        if temporary.exists() {
        ...
        File::open(&path)
            .map_err(|error| {
                if error.kind() == std::io::ErrorKind::NotFound {
                    StoreError::Missing(digest.clone())
                } else {
                    StoreError::Io(error)
                }
            })?
    ...
    pub fn remove_record(&self, digest: &Digest) -> Result<(), StoreError> {
        let path = self.record_path(digest);
        if !path.exists() {
            return Err(StoreError::Missing(digest.clone()));
        }
        fs::remove_file(path)?;
        Ok(())
    }
...
mod tests {
    ...
    fn records_are_atomically_addressed_by_manifest_digest() {
        ...
        assert_eq!(
            store.load_record(&record.manifest_digest).expect("load"),
            record
        );
        assert!(matches!(
    ...
    fn corrupt_or_mismatched_records_are_not_accepted() {
        ...
        let store = ImageStore::open(directory.path()).expect("store");
        let record = record();
        std::fs::write(store.record_path(&record.manifest_digest), b"not-json")
