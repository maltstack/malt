...
impl Env {
    ...
    /// A handle to this environment's descriptor table.
    ///
    /// The registry is `Arc`-backed, so the returned handle refers to the same
    /// table rather than a copy. The daemon holds one in order to swap fd 0
    /// when a client signals end-of-input: the reader blocked on the old pipe
    /// must see EOF, and the *next* command must find a fresh pipe waiting.
    /// Doing that through the registry keeps it off the worker thread, which
    /// owns the `Env` and is at that moment blocked inside a command.
    pub fn fd_registry(&self) -> malt_platform::vfs::SharedFdRegistry {
