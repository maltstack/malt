...
- [ ] T028 [P] [US3] **BLOCKED on privilege + base layer — the crash is fixed.** The `STATUS_ACCESS_VIOLATION` was a null `HCS_OPERATION` handle passed to `HcsStartComputeSystem` and `HcsTerminateComputeSystem`; fixed 2026-07-26, covered by `hcs_create_never_faults_whatever_this_host_supports` (no longer `#[ignore]`d). See [brief 006](../../docs/briefs/006-hcs-backend-access-violation.md). With the crash gone the real blocker is legible: `HCS_E_ACCESS_DENIED` — the daemon is not an administrator or a member of Hyper-V Administrators. Spawning a contained process therefore needs a **privileged helper** (vexil-v2 runs HCS from a LocalSystem service for exactly this reason) **and a base image layer**, neither of which is in 007's scope. Do not widen 007 to absorb them (Principle IX); they belong to the separately scoped container-substrate work.
