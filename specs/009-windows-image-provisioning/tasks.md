...
- [ ] T049 Make contained HCS process stdin lifecycle complete one-shot commands without a manual `malt eof`, while preserving explicit raw-input/EOF semantics for interactive commands, per US2/AC2 and SC-002 (partial).
- [ ] T050 Persist and report the selected immutable image digest on a contained session creation/result surface, per US2/AC1 and SC-002 (partial).
